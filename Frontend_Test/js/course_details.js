...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!courseId) {
        ...
        document.querySelector('.course_details').innerHTML = '<p>No course ID provided.</p>';
        return;
    }
    ...
    .then(course => {
        ...
        document.getElementById('courseInstructor').textContent = course.instructorName;
    ...
    loadCourseDetails();
        ...
        async function applyCoupon() {
            ...
            try {
                ...
                const response = await fetch(`http://localhost:8090/courses/${courseId}/price?couponCode=${couponCode}`);
                ...
                if (response.ok) {
                    ...
                    if (data.originalPrice > data.discountedPrice) {
                        const savingsPercentage = ((data.originalPrice - data.discountedPrice) / data.originalPrice * 100).toFixed(2);
                        discountMessageP.textContent = `You save ${savingsPercentage}%!`;
                        discountMessageP.style.color = 'green';
                    } else {
                        discountMessageP.textContent = 'Invalid or expired coupon code.';
                        discountMessageP.style.color = 'red';
                    }
                } else {
                    discountMessageP.textContent = 'Error applying coupon. Please try again.';
                    discountMessageP.style.color = 'red';
                }
            } catch (error) {
                console.error('Error:', error);
                discountMessageP.textContent = 'An error occurred. Please try again later.';
                discountMessageP.style.color = 'red';
            }
        }
...
enrollButton.addEventListener('click', async () => {
        ...
        if(payment_status=='true'){
            ...
            let countdownTime = parseInt(localStorage.getItem('countdownTime')) || initialCountdownTime;
            let endTime = localStorage.getItem('endTime') ? new Date(localStorage.getItem('endTime')) : new Date(Date.now() + countdownTime * 1000);
        ...
        document.getElementById('refundButton').addEventListener('click', handleRefundButtonClick);
        ...
        btn.onclick = function() {
            ...
            videoPlayer.innerHTML = '<iframe width="560" height="315" src="https://www.youtube.com/embed/JbTBUvyNnpY?si=HHP8QIod5AWtPbjG" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>';
        }
